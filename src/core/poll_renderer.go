...
func (this *Context) Prepare(data interface{}) *Context{
	return &Context{data, this.Show_results, this.Show_vote}
}
...
func (poll *Poll) Render(show_results bool, show_vote bool) (string, error) {
	writer := bytes.NewBuffer(nil)
	if err := templates.ExecuteTemplate(writer, "poll", &Context{poll, show_results, show_vote}); err != nil{
		return "", err
	}
